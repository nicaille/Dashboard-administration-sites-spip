...
function dashboard_synchroniser($id_dashboard_site, $options = []) {
	...
	$php_site     = (string) ($infos['serveur']['php'] ?? '');

	// L'état du core se décide sur deux avis : celui de la tour, qui lit
	// l'annuaire officiel de spip.net, et celui du site, qui a reçu le sien du
	// génie `mise_a_jour` de son propre core. La tour tranche quand elle sait ;
	// le site parle quand elle se tait, et le badge dit alors d'où ça vient.
	include_spip('inc/dashboard_spip_api');
	$core = dashboard_core_confronter(
		dashboard_core_etat($version_spip, $php_site),
		dashboard_core_avis_site($infos),
		$version_spip
	);
	$cible = (string) $core['etat'] === 'retard' ? (string) $core['cible'] : '';
	...
	sql_updateq('spip_dashboard_sites', [
		...
		'core_etat'      => (string) $core['etat'],
		'core_cible'     => (string) $core['cible'],
		'core_majeure'   => (string) $core['majeure'],
		'core_provenance' => (string) $core['provenance'],

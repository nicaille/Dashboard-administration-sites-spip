...
function formulaires_configurer_dashboard_charger_dist() {
	...
	// dans le fichier de fonctions du plugin : un seul lecteur pour le formulaire
	// et pour l'annuaire, sans quoi les deux défauts divergeraient.
...
function formulaires_configurer_dashboard_verifier_dist() {
	...
	// Vidable exprès : un parc peut choisir de n'interroger aucun service
	// extérieur. On ne contrôle donc que ce qui est renseigné.
	$annuaire = trim((string) _request('url_versions_spip'));
	if ($annuaire !== '' && !dashboard_url_acceptable($annuaire)) {
		$erreurs['url_versions_spip'] = _T('dashboard:erreur_url_versions');
	}
...
function formulaires_configurer_dashboard_traiter_dist() {
	...
	// Changer d'annuaire doit se voir tout de suite : garder l'ancien cache
	// ferait croire que la nouvelle adresse répond ce que répondait l'ancienne.
	unset($config['cache_api']);

...
function dashagent_infos_spip() {
	...
	return [
		...
		// Ce que le site sait déjà de sa propre mise à jour, et qu'il a appris
		// sans nous : le génie `mise_a_jour` du core interroge spip.net et range
		// son verdict en meta. C'est la source qui prévient le webmestre par
		// courriel, donc celle qui a une chance d'être en avance sur la tour.
		'maj_disponible'  => dashagent_maj_core_connue(),
		'maj_controlee'   => dashagent_maj_core_date(),
...
/**
 * La version que le core de ce site se sait devoir installer.
 *
 * Deux metas disent la même chose de deux façons, et aucune n'est fiable seule :
 *
 * - `derniere_maj_notifiee` porte un numéro propre, mais n'est écrite **qu'après
 *   l'envoi d'une notification** : un site dont `_MAJ_NOTIF_EMAILS` est vide, ou
 *   qui n'a aucun webmestre avec courriel, ne l'aura jamais ;
 * - `info_maj_spip` est écrite à chaque passage du génie, mais contient du
 *   **HTML** — un lien vers spip.net dont l'attribut `title` porte le numéro.
 *
 * On n'en fait jamais voyager le balisage. La tour de contrôle affiche ce que
 * ses agents répondent dans un espace privé qui détient les secrets de tout le
 * parc : ce qui part d'ici est un numéro de version, ou rien.
 *
 * @return string Version disponible selon ce site, ou chaîne vide
 */
function dashagent_maj_core_connue() {
	$propre = trim((string) ($GLOBALS['meta']['derniere_maj_notifiee'] ?? ''));
	if (preg_match('/^\d+\.\d+\.\d+[a-z0-9.\-]*$/i', $propre)) {
		return $propre;
	}

	// `info_maj_spip` vaut « <branche>|<html> », et vide quand le site est à
	// jour. On n'en retient que le premier numéro complet qui s'y trouve.
	$brut = (string) ($GLOBALS['meta']['info_maj_spip'] ?? '');

	return preg_match('/(\d+\.\d+\.\d+[a-z0-9.\-]*)/i', $brut, $m) ? $m[1] : '';
}

/**
 * Quand ce site a-t-il lu l'annuaire des versions pour la dernière fois ?
 *
 * Le génie du core ne passe que toutes les soixante-douze heures, et son verdict
 * peut donc avoir trois jours. La date est celle du fichier que
 * `info_maj_cache()` écrit, et seulement quand la lecture a réussi : c'est donc
 * bien la fraîcheur de l'avis, pas celle de la tentative.
 *
 * @return string Date au format SQL, ou chaîne vide
 */
function dashagent_maj_core_date() {
	if (!defined('_DIR_CACHE')) {
		return '';
	}
	$fichier = _DIR_CACHE . 'spip_versions_list.json';
